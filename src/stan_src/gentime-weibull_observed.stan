...
model {
    ...
    for (ID in 1 : N_IDs)
        target += neg_binomial_2_log_lupmf(epilink[ID].1 | log_R0, phi);
    target += neg_binomial_2_log_lupmf(0 | log_R0, phi); // case No. 42 has no secondary cases, so we add a zero to the likelihood

    target += weibull_lupdf(exposure[ID_infectee] - exposure[ID_infector] | param1_gentime, param2_gentime);
}
