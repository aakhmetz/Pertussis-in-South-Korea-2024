...
transformed parameters {
    ...
    real param1_gentime, param2_gentime,
    ...
    // Lognormal distribution
    {
        ...
        param2_gentime = sqrt(sqr_param2_lognormal);
        param1_gentime = log_mean_gentime - sqr_param2_lognormal / 2.0;
    }
...
model {
    ...
    for (ID in 1 : N_IDs) {
        if (exposure[ID] > t_int) {
            ...
            log_R = log_R0 + log1m_epsilon;
        } else {
            // this ID is a pre-intervention case
            log_R = log_R0 + log_sum_exp(lognormal_lcdf(t_int - exposure[ID] | param1_gentime, param2_gentime), log1m_epsilon + lognormal_lccdf(t_int - exposure[ID] | param1_gentime, param2_gentime));
        }
        target += neg_binomial_2_log_lupmf(epilink[ID].1 | log_R, phi);
    }
    target += neg_binomial_2_log_lupmf(0 | log_R0 + log1m_epsilon, phi); // case No. 42 has no secondary cases, so we add a zero to the likelihood

    target += lognormal_lpdf(exposure[ID_infectee] - exposure[ID_infector] | param1_gentime, param2_gentime);
    for (n in 1 : N) {
        if (t_int > exposure[ID_infector[n]]) {
            if (exposure[ID_infectee[n]] > t_int)
                target += log1m_epsilon;
            target += -log_sum_exp(log1m_epsilon + lognormal_lccdf(t_int - exposure[ID_infector[n]] | param1_gentime, param2_gentime), lognormal_lcdf(t_int - exposure[ID_infector[n]] | param1_gentime, param2_gentime));
        }
    }
}
...
generated quantities {
    ...
    real gentime_pred = lognormal_rng(param1_gentime, param2_gentime);
