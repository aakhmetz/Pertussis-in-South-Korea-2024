...
transformed parameters {
    ...
    real lp_incper;
    {
        ...
        // incubation periods
        lp_incper = lognormal_lpdf(onset - exposure | param1_incper, param2_incper);
...
model {
	...
	/* likelihood */
    for (ID in 1 : N_IDs) 
        target += neg_binomial_2_log_lpmf(epilink[ID].1 | log_R0, phi);
    target += neg_binomial_2_log_lpmf(0 | log_R0, phi); // case No. 42 has no secondary cases, so we add a zero to the likelihood
