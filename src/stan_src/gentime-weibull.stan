...
transformed parameters {
    ...
    // parameters of the lognormal distribution for the gentime
    ...
    param1_gentime = exp(log_param1_Weibull_gentime);
    real sqr_cv_gentime = tgamma(1 + 2 / param1_gentime) / square(tgamma(1 + 1 / param1_gentime)) - 1,
        cv_gentime = sqrt(sqr_cv_gentime),
        sd_gentime = mean_gentime .* cv_gentime,
        log_sd_gentime = log(sd_gentime);
    param2_gentime = mean_gentime / tgamma(1.0 + 1.0 / param1_gentime);
...
model {
    ...
    for (ID in 1 : N_IDs) {
        if (exposure[ID] > t_int) {
            ...
            log_Rt[ID] = log_R0 + log_sum_exp(lognormal_lcdf(t_int - exposure[ID] | param1_gentime, param2_gentime), log1m(epsilon) + lognormal_lccdf(t_int - exposure[ID] | param1_gentime, param2_gentime));
        }
    ...
    target += weibull_lpdf(exposure[ID_infectee] - exposure[ID_infector] | param1_gentime, param2_gentime);
    for (n in 1 : N) {
        if (t_int > exposure[ID_infector[n]]) {
            if (exposure[ID_infectee[n]] > t_int)
                target += log1m(epsilon);
            target += -log_sum_exp(log1m(epsilon) + weibull_lccdf(t_int - exposure[ID_infector[n]] | param1_gentime, param2_gentime), weibull_lcdf(t_int - exposure[ID_infector[n]] | param1_gentime, param2_gentime));
        }
    }
}
...
generated quantities {
    ...
    real gentime_pred = weibull_rng(param1_gentime, param2_gentime);
